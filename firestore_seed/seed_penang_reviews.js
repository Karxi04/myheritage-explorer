...
function validReviewComments(place) {
  ...
  if (place.category === 'Food') {
    return [
      `The ${first} at ${name} gave us plenty of choices and the visit fitted well into our Penang food itinerary.`,
      ...
      `The ${fourth} made ${name} a convenient meal stop. Prices felt reasonable for the area.`,
      ...
      `The visit to ${name} was enjoyable because of the ${first} and ${second}. I would include it again in a Penang itinerary.`,
    ];
  }
  ...
  if (place.category === 'Nature') {
    return [
      ...
      `The walking experience at ${name} was comfortable and there were several places to pause and enjoy the surroundings.`,
      `We combined ${name} with another nearby attraction. Travel time was reasonable and the visit felt worthwhile.`,
      `The ${first} and ${third} were the highlights of ${name}. It was one of the calmer stops in our Penang plan.`,
    ];
  }

  if (place.category === 'Art') {
    return [
      `The ${first} at ${name} created several interesting photo opportunities and the exhibits were easy to explore.`,
      `${name} had engaging ${second}. The visit added a creative element to our George Town itinerary.`,
      `I enjoyed the ${third} at ${name}. The space felt welcoming and was not difficult to navigate.`,
      `The ${fourth} made ${name} suitable for visitors who want an indoor or creative activity.`,
      `${name} was a useful art stop between heritage attractions and the visit duration felt appropriate.`,
      ...
      `${name} provided a different experience from the nearby historical sites and was worth adding to the route.`,
    ];
  }

  return [
    `The ${first} at ${name} made the visit memorable and the location fitted naturally into our George Town route.`,
    ...
    `The details at ${name} were worth observing carefully. The place added useful context to the itinerary.`,
    `We visited ${name} earlier in the day and had enough space to explore the main areas comfortably.`,
    `The ${first} and ${second} were the best parts of ${name}. I would recommend keeping it in a Penang heritage plan.`,
  ];
}
...
async function seed() {
  const removedV2 = await deleteBySource('initial_content_v2');
  const removedV3 = await deleteBySource('penang_seed_v3');
  ...
  for (const place of places) {
    contentBatch.set(db.collection('place_content').doc(place.id), {
      name: place.name,
      placeNameKey: place.name.toLowerCase(),
    ...
    const validComments = validReviewComments(place);
    for (let index = 0; index < validComments.length; index++) {
      reviewNumber += 1;
      validCount += 1;
      const rating = index < 2 ? 5 : index < 7 ? 4 : 3;
      const ref = db.collection('reviews')
          .doc(`penang_v3_${place.id}_valid_${index + 1}`);

      reviewBatch.set(ref, {
        userId: `penang_v3_user_${reviewNumber}`,
        reviewerName:
            reviewerNames[reviewNumber % reviewerNames.length],
        placeId: `seed_${place.id}`,
        placeName: place.name,
        placeNameKey: place.name.toLowerCase(),
        source: 'penang_seed_v3',
        rating,
        comment: validComments[index],
        status: 'valid',
        flagReason: null,
        flagReasons: [],
        seededForTesting: true,
        createdAt: Timestamp.fromDate(
          new Date(now - reviewNumber * 10 * 60 * 60 * 1000),
        ),
        updatedAt: Timestamp.now(),
      });
    }
    ...
    const flagged = [
      ...
      },
      {
        rating: 3,
        comment:
            `${repeatedWord} ${repeatedWord} ${repeatedWord} ${repeatedWord} ${repeatedWord}`,
    ...
    for (let index = 0; index < flagged.length; index++) {
      ...
      const ref = db.collection('reviews')
          .doc(`penang_v3_${place.id}_flagged_${index + 1}`);

      reviewBatch.set(ref, {
        userId: `penang_v3_flag_user_${reviewNumber}`,
        reviewerName:
            reviewerNames[reviewNumber % reviewerNames.length],
        placeId: `seed_${place.id}`,
        placeName: place.name,
  ...
  console.log('');
  console.log('Penang review data created successfully.');
  console.log(`Removed old v2 records: ${removedV2}`);
  console.log(`Removed old v3 records: ${removedV3}`);
  ...
  console.log('');
  console.log(
    'Open Admin > Review Moderation > Flagged to test NLP-assisted moderation.',
  );
}

seed().then(() => process.exit(0)).catch((error) => {
  console.error('Penang review seed failed:', error);
  process.exit(1);
});
