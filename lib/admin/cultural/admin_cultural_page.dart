  ...
  Future<void> moderateSubmission(
    ...
    if (approve) {
      ...
      await AppServices.db.runTransaction((transaction) async {
        final travelerRef = AppServices.travelerRef(userId);
        final submissionSnapshot = await transaction.get(document.reference);
        final travelerSnapshot = await transaction.get(travelerRef);

        if (!submissionSnapshot.exists ||
            submissionSnapshot.data()?['status'] != 'pending') {
          throw Exception('This submission has already been reviewed.');
        }

        if (!travelerSnapshot.exists ||
            travelerSnapshot.data()?['role'] != 'traveler') {
          throw Exception('Traveler profile was not found.');
        }

        final currentPoints =
            ((travelerSnapshot.data()?['points'] ?? 0) as num).toInt() + points;
        final impact =
            ((travelerSnapshot.data()?['localImpactScore'] ?? 0) as num)
                .toInt() +
            points;
        ...
        children: [
            ...
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return ExplorerEmptyState(
                  title: 'Unable to load cultural tasks',
              ...
              if (docs.isEmpty) {
                return emptyState('No cultural tasks');
              }
          ...
          StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return ExplorerEmptyState(
                  title: 'Unable to load task submissions',
                  subtitle: '${snapshot.error}'.replaceFirst('Exception: ', ''),
              ...
              if (docs.isEmpty) {
                return emptyState('No pending task submissions');
              }
                        ...
                        children: [
                          ...
                          Column(
                            children: [
                              FilledButton.icon(
                                onPressed: () =>
                                    moderateSubmission(context, document, true),
                                icon: const Icon(Icons.check),
                                label: const Text('Approve'),
                              ),
                              const SizedBox(height: 8),
                              OutlinedButton.icon(
                                onPressed: () => moderateSubmission(
                                  context,
                                  document,
                                  false,
                                ),
...
class _TaskEditorState extends State<_TaskEditor> {
  ...
  final photoCategory = TextEditingController(text: 'heritage_feature');
  final points = TextEditingController(text: '120');
  final learningOutcome = TextEditingController(
    text: 'Learn one verified fact about Penang history or built heritage.',
  );
  ...
  void _applyCategory(String selected) {
    ...
    setState(() {
      ...
      photoCategory.text = '${details['photoCategory']}';
      points.text = '${details['points']}';
      learningOutcome.text = '${details['learningOutcome']}';
    });
  }
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                labelText: 'Description and traveler instructions',
              ),
            ),
            const SizedBox(height: 10),
            ...
            FutureBuilder<QuerySnapshot<Map<String, dynamic>>>(
              future: AppServices.db.collection('vendors').get(),
              builder: (context, snapshot) {
                if (!snapshot.hasData) {
                ...
                final vendors =
                    snapshot.data!.docs.where((doc) {
                      ...
                      (first, second) => '${first.data()['businessName'] ?? ''}'
                          .compareTo('${second.data()['businessName'] ?? ''}'),
                    );
                ...
                return DropdownButtonFormField<String>(
                  ...
                  items: vendors
                      ...
                      .toList(),
                  onChanged: (value) {
                    final document = vendors.firstWhere(
                      (doc) => doc.id == value,
                    );

                    setState(() {
                      selectedVendorId = value;
                      selectedVendor = document.data();
                    });
                  },
                );
              },
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                labelText: 'Required photo evidence category',
              ),
            ),
            const SizedBox(height: 10),
            TextField(
    ...
    actions: [
      ...
      FilledButton(
        onPressed: () {
          ...
          final reward = int.tryParse(points.text) ?? 0;
