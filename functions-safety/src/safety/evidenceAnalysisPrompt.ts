...
export function buildAnalysisPrompt(params: {
  ...
  const systemInstruction = [
    ...
    'and to assess whether the community evidence photo exhibits visual indicators of synthetic or AI-generated imagery.',
    '',
    ...
    '  "syntheticImageRisk": <"LOW" | "UNCERTAIN" | "ELEVATED">,',
    '  "syntheticImageConfidence": <number 0.0-1.0>,',
    '  "syntheticImageSummary": <string, one sentence, max 200 chars>',
    '}',
  ].join('\n');
  ...
  const textPreamble = [
    ...
    '',
    '6. syntheticImageRisk ("LOW" | "UNCERTAIN" | "ELEVATED"):',
    '   Assess ONLY the community evidence photo for synthetic risk.',
    '   Look for visual indicators such as inconsistent object geometry, impossible or irregular textures,',
    '   repeated synthetic patterns, malformed text/signage, inconsistent lighting/shadows, unnatural edges,',
    '   physically implausible structures, excessive generative smoothing, or suspicious duplicated visual elements.',
    '   RULES:',
    '   - These indicators are NOT definitive proof.',
    '   - Normal image compression, blur, low light, panorama distortion, camera processing, or editing filters can produce similar artifacts.',
    '   - If evidence is insufficient or mixed, return UNCERTAIN.',
    '   - Do NOT infer authenticity simply because no obvious artifact is visible.',
    '   - Do NOT classify based on camera/gallery source alone.',
    '   MAPPING:',
    '   - LOW: No strong visible synthetic indicators detected. (Does NOT mean authentic).',
    '   - UNCERTAIN: Image quality or visual evidence is insufficient for a reliable assessment, or mixed indicators exist.',
    '   - ELEVATED: Multiple visible anomalies appear consistent with synthetic/generated or heavily manipulated imagery. (Does NOT prove fake).',
    '',
    '7. syntheticImageConfidence (0.0-1.0):',
    '   Confidence in your syntheticImageRisk assessment (NOT the probability that the image is fake).',
    '',
    '8. syntheticImageSummary (string):',
    '   One concise sentence explaining the visual synthetic-risk assessment for administrator review. Max 200 characters.',
  ].join('\n');
    ...
    config: {
      ...
      responseSchema: {
        ...
        properties: {
          ...
          syntheticImageRisk: {
            type: Type.STRING,
            enum: ['LOW', 'UNCERTAIN', 'ELEVATED'],
          },
          syntheticImageConfidence: {
            type: Type.NUMBER,
            description: 'Confidence in risk assessment 0-1',
          },
          syntheticImageSummary: {
            type: Type.STRING,
            description: 'One-sentence synthetic risk explanation',
          },
        },
        required: [
          'sceneMatchScore',
