...
void main() {
  ...
  group('DuplicateHazardWarningSheet widget tests', () {
    ...
    testWidgets(
      'renders pending review badge and kilometer distances correctly',
      (tester) async {
        final sample = createSampleReport(
          id: 'hz-2',
    ...
    testWidgets(
      'caps displayed candidates at 3 and shows remaining count note',
      (tester) async {
    ...
    for (final width in [320.0, 390.0]) {
      for (final scale in [1.0, 2.0]) {
        testWidgets(
          'responsive layout without overflow at width $width and scale $scale',
          (tester) async {
  ...
  group('CreateHazardPage duplicate detection integration tests', () {
    ...
    testWidgets(
      ...
      (tester) async {
        setupScreen(tester);
        final mockService = MockHazardReportService()..candidatesToReturn = [];
        ...
        expect(
          mockService.lastCreatedReportParams?['description'],
          'Water pipe leaking on footpath',
        );
